# package_ip.tcl -- packages gmsk_step2d_gardner_ted.v as a reusable Vivado IP
#
...
#
# Output lands in ./ip_repo/gmsk_step2d_gardner_ted -- add that path as an IP
# Repository (alongside gmsk_step0_regs/gmsk_step1_discriminator/
# gmsk_step2a_interpolator/gmsk_step2b_nco) in the fmcomms2_zed Vivado
# project before instantiating it in the block design.

set script_dir [file normalize [file dirname [info script]]]
set ip_name    gmsk_step2d_gardner_ted
...
# Same ASSOCIATED_BUSIF fix gmsk_step1_discriminator's/gmsk_step2a_interpolator's
# packaging needed -- Vivado's auto-inference only associates aclk with
# whichever bus interface it finds first (observed: just m_axis). Both
# s_axis and m_axis run off aclk; Connection Automation uses
# ASSOCIATED_BUSIF to know that.
...
set_property display_name       "GMSK Step 2d Gardner TED"    $core
set_property description        {Gardner timing-error detector core -- e(n) = (y_on(n)-y_on(n-1))*y_mid(n), standalone ahead of the loop filter and closed timing loop.} $core
