...
**Not yet done -- exactly where to resume:** a rebuild with the SRL fix applied is in progress as of this entry (background batch job, using the working script pattern above). Check `runme.log`'s ATF result once it completes -- if still a large violation, check whether the failing path is a *third* distinct issue or a residual instance of one already covered. Once genuinely clean: verify `system_top.ltx` shows `probe0`/`probe2` as 24-bit, redeploy via `BOOT.BIN`, capture the interpolator's real output on hardware. Also still outstanding: committing this session's newest `ADI_hdl` state (the two interpolator RTL fixes' repackaged IP reaching the block design) once a clean build confirms it's worth committing.
