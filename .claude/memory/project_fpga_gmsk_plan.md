...
**Tested the hypothesis directly:** copied `rx.c`/`rf_params.h` to the ZedBoard (`192.168.1.104`, current IP as of today — see [[project_sdr_devices]]), compiled natively (`gcc -O2 -liio -lm`), ran it. **Reproduced last session's exact behavior: PHY config step ("Configuring AD9361 RF front-end...", RX LO/buffer creation) completed successfully, then it failed at the same `iio_buffer_refill error: -110`, process exited cleanly (exit code 0).** This confirms PHY bring-up genuinely runs to completion on this exact bitstream — the failure is isolated specifically to the DMA/`iio_buffer_refill` step, not PHY init. Whether this PHY-config pass actually released `axi_ad9361_rst` — and whether that release persists after the app exits and its IIO context closes, or gets torn back down — is the very next thing to check via a fresh Trigger-Immediate ILA capture taken after this run. **This is exactly where to resume if the session drops:** check the post-`rx.c` ILA capture result before doing anything else.
