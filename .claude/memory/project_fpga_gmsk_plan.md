...
**Not yet done — exactly where to resume:** re-add the fixed discriminator IP in the block design (Vivado should offer to refresh/upgrade the existing instance in place from the updated `component.xml` — port list/interfaces are unchanged, only internal logic, so no rewiring should be needed), regenerate output products, rebuild, redeploy via `BOOT.BIN` (same recipe, `hdl/boot_build/`), and re-run the live TX/RX + ILA capture to confirm the fix on real hardware — this time checking the raw exported CSV data (valid-gated) before declaring success, not just the waveform view.
