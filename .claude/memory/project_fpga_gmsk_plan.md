...
**Step 1a is essentially answered and then some** — not only is the debug-capture chain proven sound (the original goal), the exercise surfaced and hardware-confirmed the actual root cause of Step 1's original DMA mystery. **Next step, exactly where to resume if the session drops:** with `cf_axi_adc` freshly rebound and `up_resetn` now confirmed released, re-run `rx.c` and check whether `iio_buffer_refill`'s `-110` is now gone. If DMA now works, the whole Step 1 hardware validation (discriminator against real AD9361 RX data) is unblocked — the fix going forward is simply "JTAG-program, then unbind/rebind `cf_axi_adc`, before running any RX software," until/unless a persistent `BOOT.BIN` with the custom bitstream baked in is built (removing the need for a live JTAG swap entirely).
