...
**Second rebuild in progress as of session end — not yet redeployed/re-verified on hardware.** Once it completes: check `system_top.ltx` shows `probe0[23:0]` before redeploying (don't repeat the same false-start), then redeploy via `BOOT.BIN` and recapture.

**Separately, Pari did get a clean hardware capture on the (still 16-bit-probe) build**: real 1,0,1,0 visibly toggling on the discriminator output via ILA — a genuine, human-confirmed pass of Step 1's own test gate, independent of the width-fix verification track above.
