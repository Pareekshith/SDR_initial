...
**Where this leaves the investigation:** the robust fix is almost certainly to stop live-JTAG-swapping the bitstream into an already-booted kernel altogether, and instead bake `system_top.bit` into a persistent `BOOT.BIN` (via a matching FSBL export + `bootgen`) so it's present in the fabric from cold boot, before any driver ever probes — eliminating this entire class of "driver configured against stale/wrong bitstream state" bug for both `axi_ad9361` and `axi_dmac` at once, permanently. This has been an outstanding TODO since 2026-07-19 (see [[project_sdr_devices]] item 2). The faster but less certain alternative is finding `axi_dmac`'s `ctrl_enable`-equivalent register bit and writing it directly via `devmem` (same technique as `up_resetn`, but for the DMA core), sidestepping the crash-prone unbind/`.remove()` path entirely — not yet attempted. **This is exactly where to resume if the session drops:** decide between building a persistent `BOOT.BIN` (permanent fix, more upfront work) vs. a targeted `devmem` register poke for `axi_dmac` (faster, narrower, unconfirmed).
