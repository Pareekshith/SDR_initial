...
**Not yet confirmed on hardware:** the IP in the live `fmcomms2_zed` Vivado project needs to pick up this updated source (right-click the IP repo / `gmsk_step1_discrimin_0` -> Refresh, or Vivado may auto-detect via "Report IP Status" -> Upgrade), then Generate Output Products + Generate Bitstream again. **This is exactly where to resume if the session drops:** rebuild pending, watch `runme.log` for the ATF/timing result again before trusting a WNS pass this time (this specific path might still be tight even with the extra stage — if it fails again, the next lever is a Vivado placement constraint pulling `gmsk_step1_discrimin_0` physically closer to `axi_ad9361`, not further RTL changes).
