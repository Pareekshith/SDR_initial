...
## Both boards re-verified reachable (2026-08-23), ZedBoard IP changed again as expected

**ZedBoard now `192.168.1.100`** (DHCP reassigned again, as always happens on reboot -- confirmed via MAC `00:0a:35:00:01:23` match in `arp -a`). SSH host key **unchanged** since 2026-07-19 (`SHA256:BNeiBArC949eFL23J+no8BfYq6rHikzDuaLiaiMuIzo`), confirming it's genuinely the same board/rootfs, not a reflash. `hostname` returns `analog`. SD card healthy (`dmesg` clean of `mmcblk0` errors, `/boot` has 750M free).

**Gotcha hit and resolved while verifying:** the ZedBoard's SSH banner now reads `SSH-2.0-OpenSSH_8.4p1 Raspbian-5+deb11u4` -- looked alarming at first glance (a Raspberry-Pi-flavored banner on a Zynq board reads like a possible LAN impersonation, exactly the failure mode this file already documents once for Pluto+'s old `.102` IP). **Resolved by comparing the negotiated host key fingerprint against the recorded one instead of trusting the banner string alone** -- exact match, so it's genuinely our board. The "Raspbian" string is just an artifact of Kuiper's shared Debian-based packaging across all the boards it supports (Kuiper explicitly covers Raspberry Pi alongside Zynq/ZynqMP/etc, per the earlier reflash notes) -- not a sign of anything wrong. **Lesson: verify device identity by host-key fingerprint match, not by OS banner text, which can vary across a shared multi-board image without meaning anything's wrong.**

**Pluto+ confirmed at `192.168.1.50`** as expected (static IP held). Its SSH host key rotated since the 2026-08-01 note (`SHA256:ZfuqQpjXt2HRRUfiSrc4uNLBZ/ScacJW+fMgTnG/VmE` -> now `SHA256:dolpjaftuo2ByES119uSiYCmPbO2l3ITfqexY6srDKk`, ECDSA not RSA -- dropbear offers both, plink negotiates ECDSA by default) -- unlike the ZedBoard case this key DID change, presumably from a reboot/dropbear regenerating it; MAC/IP/OS-banner (`dropbear_2019.78`, matches Buildroot) all still consistent with genuine Pluto+, so treated as a normal key rotation, not a concern. `hostname` returns `pluto`.

**Practical note for `plink` non-interactive host-key pinning:** `-batch -hostkey "SHA256:..."` requires the fingerprint of whichever key type the server and plink actually NEGOTIATE (ECDSA/Ed25519 preferred over RSA by default), not just any key the server happens to offer -- `ssh-keyscan`'s output isn't reliable for this since it may report a different key type than what a real `plink` connection picks. Get the real negotiated fingerprint via `plink -v -ssh -batch -hostkey "<any validly-formatted placeholder>" root@<ip> ...` first (fails safely before sending any password) and read the fingerprint from its verbose output, then use that exact value for the real login. Plain `echo y | plink ...` / stdin-redirect tricks to auto-accept an unknown key did NOT work in this environment (plink's interactive prompt appears to write to the raw console, not to redirectable stdout, so these hung indefinitely) -- `-batch` + the correct `-hostkey` is the reliable non-interactive path.
